...
private fun discoverNeighbour(input: List<List<Char>>, startingPoint: Pair<Int, Int>): List<String> {
    ...
    while (true) {
        val nbs = mapOf(
            "${currRPosition - 1}:${currCPosition}:north" to pipes[input.getOrNull(currRPosition - 1)
                ?.getOrNull(currCPosition)], // top
            "${currRPosition + 1}:${currCPosition}:south" to pipes[input.getOrNull(currRPosition + 1)
                ?.getOrNull(currCPosition)], // bottom
            "${currRPosition}:${currCPosition - 1}:west" to pipes[input.getOrNull(currRPosition)
                ?.getOrNull(currCPosition - 1)], // left
            "${currRPosition}:${currCPosition + 1}:east" to pipes[input.getOrNull(currRPosition)
                ?.getOrNull(currCPosition + 1)] // right
        )
...
private fun part1(input: MutableList<MutableList<Char>>): Int {
    ...
    input[startingPoint.first][startingPoint.second] =
        'F' // We should write some code to automatically detect where it should start
...
private fun part2(input: MutableList<MutableList<Char>>): Int {
    ...
    for (i in input.indices) {
        for (j in input[i].indices) {
            ...
            if (isInside(polygon, testPoint) && !polygon.contains(testPoint)) {
                    count++
            }
        }
    }
...
fun main() {
    ...
    val testInput2 = parseInput(readInput("Day10_test2"))
    check(part2(testInput2) == 4)

    val input = parseInput(readInput("Day10"))
    check(part1(input) == 7173)
    check(part2(input) == 291)
}
